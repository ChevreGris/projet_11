class TestAuth:
    ...
    def test_valid_booking_should_display_confirmation_message(self, client, test_club, test_competition):
        ...
        response = client.post('/purchasePlaces', data={"competition": test_competition["name"], "club": test_club['name'], "places": places}, follow_redirects=True)
    ...
    def test_shouldnt_book_when_0_points(self, client, test_club,  zero_point_club):
        response = client.get(f'/book/{test_club["name"]}/{zero_point_club["name"]}')
